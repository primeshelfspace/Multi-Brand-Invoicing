...
const envSchema = z
  .object({
    ...
    // CORS_ALLOWED_ORIGINS (the old per-origin allowlist input) is gone —
    // main.ts now sets origin: true and accepts every origin, so there's
    // nothing left for this to feed into.
